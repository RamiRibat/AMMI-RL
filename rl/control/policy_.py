...
class StochasticPolicy(nn.Module):
	...
	def pi_prob(self, mean, std, reparameterize, return_log_prob):
		...
		prob, log_prob, entropy = T.tanh(sample), None, None # June
	...
	def forward(self,
		...
		if deterministic: # Evaluation
			...
			pi, log_pi, entropy = self.pi_prob(mean, std, reparameterize, return_log_pi) # June
...
class StochasticPolicyJune(nn.Module):
