...
class OnPolBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size, seed, device, gamma=0.99, lam=0.95):
        self.obs_buf = np.zeros(combined_shape(size, obs_dim), dtype=np.float32)
        self.act_buf = np.zeros(combined_shape(size, act_dim), dtype=np.float32)
        self.rew_buf = np.zeros(combined_shape(size, 1), dtype=np.float32)
        self.ret_buf = np.zeros(combined_shape(size, 1), dtype=np.float32)
        self.val_buf = np.zeros(combined_shape(size, 1), dtype=np.float32)
        self.adv_buf = np.zeros(combined_shape(size, 1), dtype=np.float32)
        self.log_pi_buf = np.zeros(combined_shape(size, 1), dtype=np.float32)
        self.ptr, self.traj_start_inx, self.max_size = 0, 0, size
        sself.gamma, self.lambda = gamma, lam


    def store_transition(self, obs, act, rew, val, log_pi):
        assert self.ptr < self.max_size

        self.obs_buf[self.ptr] = obs
        self.act_buf[self.ptr] = act
        self.rew_buf[self.ptr] = rew
        self.val_buf[self.ptr] = obs_next
        self.log_pi_buf[self.ptr] = ter
        self.ptr +=1
