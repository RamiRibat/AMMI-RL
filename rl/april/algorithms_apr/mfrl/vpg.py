...
class VPG(MFRL):
    ...
    def learn(self):
        ...
        for n in range(1, N + 1):
            ...
            # logs['training/vpg/Jv                '] = np.mean(JVList)
            # logs['training/vpg/Jpi               '] = np.mean(JPiList)
            #
...
def main(exp_prefix, config, seed, device, wb):
    ...
    agent = VPG(exp_prefix, configs, seed, device, wb)

    agent.learn()

    print('\n')
    print('... End the VPG experiment')
